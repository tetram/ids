...
#
# Adapte et mofifie pour le projet IDS
...
class ViewerGtk:
    ...
    FULLSCREEN = True
    WALK_INSTEAD_LISTDIR = False # walk instead list dir not supported yet
    ...
    def load_file_list(self):
        ...
        if self.WALK_INSTEAD_LISTDIR:
            for directory, sub_directories, files in os.walk(self.pic_dir):
                for filename in files:
                    if is_image(filename):
                        filepath = os.path.join(directory, filename)
                        self.files.append(filepath)
        else:
            print(os.listdir(self.pic_dir))
            for filename in os.listdir(self.pic_dir):
		pic_path = self.pic_dir+'/'+filename
                if is_image(pic_path):
                    self.files.append(pic_path)
